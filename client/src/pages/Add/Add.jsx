...
const Add = () => {
  ...
  const handleImageUploads = async () => {
    try {
      ...
      // If `coverImage` is optional, this will either upload the image or use the default URL
      const cover = await generateImageURL(coverImage);

      // For multiple images, map through each, and if an image is missing, it will get the default URL
      const images = await Promise.all(
        [...gigImages].map(async (img) => await generateImageURL(img || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6cl1MISDvIR9C5YIw_J-uaMhiOaQRSwC7Lw&s"))
      );
